...
// @updateURL   https://raw.githubusercontent.com/oligochrome/Userscripts/master/SMMO/Inventory%20%5C%20Trade%20Item%20Counter
// @grant       none
// @version     1.0
// @author      Ogliochrome
...
if (url.includes('trades/view/')) {
    for (let index = 0; index < y.length; index++) {
        if (y[index].childElementCount === 0) {
            ...
            switch (rarity) {
                case "common":
                    itemc = itemc + parseInt(y[index].innerText)
                    break;
                case "uncommon":
                    itemuc = itemuc + parseInt(y[index].innerText)
                    break;
                case "rare":
                    itemr = itemr + parseInt(y[index].innerText)
                    break;
                case "elite":
                    iteme = iteme + parseInt(y[index].innerText)
                    break;
                case "epic":
                    itemep = itemep + parseInt(y[index].innerText)
                    break;
                case "legendary":
                    iteml = iteml + parseInt(y[index].innerText)
                    break;
                case "celestial":
                    itemcel = itemcel + parseInt(y[index].innerText)
                    break;
            }
...
`)
    ...
    document.getElementById('textq').innerHTML = "Common: " + itemc + "<br /> Uncommon: " + itemuc + "<br /> Rare: " + itemr + "<br /> Elite: " + iteme + "<br />Epic: " + itemep + "<br /> Legendary: " + iteml + "<br /> Celestial: " + iteml + "<br /><br /> Total: " + itemtotal + ""
